-- Migration: Layer 0 PDF Optimization Tables
...
-- Indexes for pdf_cache
CREATE INDEX idx_pdf_cache_last_accessed ON pdf_cache(last_accessed DESC);
CREATE INDEX idx_pdf_cache_created ON pdf_cache(created_at DESC);
CREATE INDEX idx_pdf_cache_document_type ON pdf_cache(document_type);
...
-- Indexes for layer0_cost_tracking
CREATE INDEX idx_cost_tracking_date ON layer0_cost_tracking(created_at DESC);
CREATE INDEX idx_cost_tracking_hash ON layer0_cost_tracking(pdf_hash);
CREATE INDEX idx_cost_tracking_document ON layer0_cost_tracking(document_id);
CREATE INDEX idx_cost_tracking_user ON layer0_cost_tracking(user_id);
CREATE INDEX idx_cost_tracking_cache_hit ON layer0_cost_tracking(cache_hit);

-- Materialized View for Statistics
-- Provides fast access to aggregate statistics
...
-- GRANT SELECT, INSERT, UPDATE ON pdf_cache TO app_user;
-- GRANT SELECT, INSERT ON layer0_cost_tracking TO app_user;
-- GRANT SELECT ON layer0_stats TO app_user;
